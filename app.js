...
app.listen(process.env.PORT || 5000, () => {
  console.log(
    "The server is now ready and is listening for requests or port number : 5000"
  );
});
...
app.post("/signup", async (req, res, next) => {
  try {
    ...
    req.login(registeredUser, (err) => {
      ...
      req.flash(
        "success",
        "User registered successfully. Welcome to Wanderly !!!"
      );
    ...
    req.flash(
      "error",
      "A user with any one of these credentials already exists !!!"
    );
    res.redirect("/signup");
  }
});
...
app.get(
  ...
  wrapAsync(async (req, res, next) => {
    ...
    res.render("allListings", {
      ...
      styles: [
        "/css/allListingsStyle.css",
        "https://cdn.jsdelivr.net/npm/bootstrap@5.3.7/dist/css/bootstrap.min.css",
      ],
    }); // here we are passing the page specific title and stylesheet so that it can be applied to that page only while the common styles will apply to all the pages
  })
);
...
app.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    res.render("showPassengerDetails", {
      title: "Verify passenger details",
      listing,
      passengers,
      styles: [
        "/css/showPassengerDetailsStyle.css",
        "https://cdn.jsdelivr.net/npm/bootstrap@5.3.7/dist/css/bootstrap.min.css",
      ],
      hideOverlay: true,
    });
...
app.get(
  ...
  wrapAsync(async (req, res, next) => {
    ...
    if (!listing) {
      req.flash(
        "error",
        "The property you are trying to access was either deleted or there was some error in fetching it"
      );
      res.redirect(`/listings`);
    } else {
      res.render("showStay", {
        listing,
        title: `Explore : ${listing.title}`,
        styles: ["/css/showStayStyle.css"],
        hideOverlay: false,
      });
    }
  })
...
app.get("/listings/:id/reviews", async (req, res) => {
  ...
  if (!listing) {
    req.flash(
      "error",
      "There was some error in finding the listing you are looking for"
    );
...
app.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (!listing) {
      req.flash(
        "error",
        "There was some error in fetching the listing you are looking for...."
      );
      res.redirect(`/listings/${id}/reviews`);
    } else {
      await review.save();
  ...
  wrapAsync(async (req, res) => {
    ...
    if (!listing) {
      ...
      if (!review) {
        req.flash(
          "error",
          "Either this review was deleted or there was some error fetching it..."
        );
      } else {
...
app.get(
  "/listings/:id/reviews/:reviewId/edit",
  isAuthenticated,
  wrapAsync(async (req, res) => {
    let { id, reviewId } = req.params;
    let listing = await Listing.findById(id);
    if (!listing) {
      req.flash(
        "error",
        "There was error in fetching the listing you are looking for at the moment !!!"
      );
...
app.use((err, req, res, next) => {
  ...
  res
    .status(status)
    .send(
      `<h3>Something went wrong !!!</h3><br><p>Error ${status}: ${message}</p>`
    );
});
